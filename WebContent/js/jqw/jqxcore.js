...
(function(f) {
	...
	var e = "Boolean Number String Function Array Date RegExp Object Error"
			.split(" ");
	...
	h.type = function(j) {
		...
		return typeof j === "object" || typeof j === "function" ? b[g.call(j)]
				|| "object" : typeof j
	};
	h.isPlainObject = function(m) {
		...
		try {
			if (m.constructor && !a.call(m, "constructor")
					&& !a.call(m.constructor.prototype, "isPrototypeOf")) {
				return false
			}
		} catch (l) {
			return false
	...
	h
			.extend({
				...
				Event : function(k, q) {
					...
					k.isTrigger = true;
					k.namespace = l.join(".");
					k.namespace_re = k.namespace ? new RegExp("(^|\\.)"
							+ l.join("\\.(?:.*\\.|)") + "(\\.|$)") : null;
					n = v.indexOf(":") < 0 ? "on" + v : "";
					k.result = undefined;
					if (!k.target) {
				...
				isRendered : function() {
					var j = this;
					if (j.element.parentNode == null
							|| (j.element.offsetWidth === 0 || j.element.offsetHeight === 0)) {
						return false
					}
					return true
				},
				...
				sizeStyleChanged : function(l) {
					...
					var j = function(n) {
						var o = m;
						if (n && n[0] && n[0].attributeName === "style"
								&& n[0].type === "attributes") {
							if (o.element.offsetWidth !== o.offsetWidth
									|| o.element.offsetHeight !== o.offsetHeight) {
				...
				prepend : function(j) {
					if (j.nodeType === 1 || j.nodeType === 11) {
						this.element
								.insertBefore(elem, this.element.firstChild)
					}
				},
				appendTo : function(j) {
					var k = this.detach();
				...
				hasClass : function(j) {
					...
					var l = " " + j + " ";
					var k = /[\t\r\n]/g;
					if (m.element.nodeType === 1
							&& (" " + m.element.className + " ")
									.replace(k, " ").indexOf(l) >= 0) {
						return true
					}
					return false
				},
				...
				html : function(v) {
					...
					try {
						...
						if (v === undefined) {
							return z.nodeType === 1 ? z.innerHTML.replace(
									rinlinejQuery, "") : undefined
						}
						var n = /<(?:script|style|link)/i, j = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video", q = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi, m = /<([\w:]+)/, C = /<(?:script|object|embed|option|style)/i, r = new RegExp(
								"<(?:" + j + ")[\\s/>]", "i"), A = /^\s+/, u = {
						...
						var t = document.createElement("div"), k = document
								.createDocumentFragment(), s = document
								.createElement("input");
				...
				isEmpty : function(j) {
					...
					j = that.trim(j).replace(/\&nbsp\;/ig, "").replace(
							/\&#160\;/ig, "");
					return j === ""
				},
				text : function() {
				...
				val : function(o) {
					...
					if (!arguments.length) {
						if (m) {
							k = m.value;
							var l = typeof k === "string" ? k.replace(n, "")
									: k == null ? "" : k;
							return l
						}
						return
				...
				boxModel : function(k, j, p) {
					...
					for (; l < 4; l += 2) {
						...
						if (p) {
							...
							if (j !== "margin") {
								o -= parseFloat(m
										.css("border" + n[l] + "Width")) || 0
							}
						} else {
							o += parseFloat(m.css("padding" + n[l])) || 0;
							if (j !== "padding") {
								o += parseFloat(m.css(this.element, "border"
										+ n[l] + "Width")) || 0
							}
						}
					}
				...
				width : function(j) {
					...
					if (k.element.nodeType === 9) {
						...
						return Math.max(k.element.body.scrollWidth,
								m.scrollWidth, k.element.body.offsetWidth,
					...
					l += k.boxModel("width", "content"
							|| (n ? "border" : "content"), true);
					return l
				},
				innerWidth : function() {
					var l = this;
				...
				height : function(j) {
					...
					if (k.element.nodeType === 9) {
						var m = k.element.documentElement;
						return Math.max(k.element.body.scrollHeight,
								m.scrollHeight, k.element.body.offsetHeight,
								m.offsetHeight, m.clientHeight)
					}
					var n = this.element.style.boxSizing == "border-box";
					if (j) {
						if (!isNaN(j)) {
				...
				css : function(k) {
					...
					if (f.getComputedStyle) {
						...
						if (document.documentElement.currentStyle) {
							j = function(t, r) {
								var u, p, q = t.currentStyle
										&& t.currentStyle[r], s = t.style;
								if (q == null && s && s[r]) {
									q = s[r]
								}
				...
				offset : function(o) {
					...
					var s = function(G) {
						return z(G) ? G : G.nodeType === 9 ? G.defaultView
								|| G.parentWindow : false
					};
					q = s(F);
					var y = 0;
					var k = 0;
					var j = navigator.userAgent.toLowerCase();
					var w = j.indexOf("ipad") != -1
							|| j.indexOf("iphone") != -1;
					if (w) {
						y = 2
					}
					return {
						top : k + v.top + (q.pageYOffset || B.scrollTop)
								- (B.clientTop || 0),
						left : y + v.left + (q.pageXOffset || B.scrollLeft)
								- (B.clientLeft || 0)
					}
				},
				position : function() {
					...
					var l = m.element, k = h(function() {
						...
						while (n
								&& (!j.test(n.nodeName) && h(n).css("position") === "static")) {
							n = n.offsetParent
						}
						return n || document.body
					}());
					offset = m.offset(), parentOffset = j.test(k.nodeName) ? {
						top : 0,
						left : 0
					} : k.offset();
				...
				contains : function(k, j) {
					var m = k.nodeType === 9 ? k.documentElement : k, l = j
							&& j.parentNode;
					return k === l
							|| !!(l && l.nodeType === 1 && m.contains && m
									.contains(l))
				},
				...
				showHide : function(q) {
					...
					var j = function(t, s) {
						t = s || t;
						return n.css(t, "display") === "none"
								|| !n.contains(t.ownerDocument, t)
					};
					r = n.data("olddisplay");
					p = l.style.display;
					if (q) {
				...
				isArraylike : function(m) {
					...
					return j === "array" || k === 0 || typeof k === "number"
							&& k > 0 && (k - 1) in m
				},
				each : function(p, q, k) {
					var n = this;
				...
				initAnimate : function() {
					...
					var s = (function() {
						...
						return f.requestAnimationFrame
								...
								|| f.mozRequestAnimationFrame || function(I) {
					...
					function k(K, O, I, N) {
						...
						function H(ah, af, ag) {
							return ((R(af, ag) * ah + Q(af, ag)) * ah + P(af))
									* ah
						}
						function ad(ah, af, ag) {
							return 3 * R(af, ag) * ah * ah + 2 * Q(af, ag) * ah
									+ P(af)
						}
						function G(ai, ag) {
						...
						function Y(aj) {
							...
							--ai;
							var al = (aj - M[ai]) / (M[ai + 1] - M[ai]), ah = ak
									+ al * ab, ag = ad(ah, K, I);
							if (ag >= J) {
								return G(aj, ah)
							} else {
					...
					var z = (function() {
						...
						function I(O, M) {
							var K = {
								dx : O.v,
								dv : H(O)
							}, J = F(O, M * 0.5, K), Q = F(O, M * 0.5, J), P = F(
									O, M, Q), N = 1 / 6 * (K.dx + 2
									* (J.dx + Q.dx) + P.dx), L = 1 / 6 * (K.dv
									+ 2 * (J.dv + Q.dv) + P.dv);
							O.x = O.x + N * M;
							O.v = O.v + L * M;
							return O
						...
						return function G(Q, K, L) {
							var T = {
								...
								tension : null,
								friction : null
							}, S = [ 0 ], M = 0, O = 1 / 10000, N = 16 / 1000, P, J, R;
							Q = parseFloat(Q) || 500;
							K = parseFloat(K) || 20;
							L = L || null;
							T.tension = Q;
					...
					E.Easings = {
						...
						spring : function(F) {
							return 1 - (Math.cos(F * 4.5 * Math.PI) * Math
									.exp(-F * 6))
						}
					};
					q
					...
					var v = E.CSS = {
						...
						Hooks : {
							...
							extractValue : function(I, H) {
								...
								if (J) {
									...
									return H.toString().match(
											v.RegEx.valueSplit)[F]
								} else {
									return H
								}
							},
						...
						Normalizations : {
							registered : {
								...
								blur : function(I, H, G) {
									switch (I) {
									case "name":
										return E.State.isFirefox ? "filter"
												: "-webkit-filter";
									case "extract":
										var F = parseFloat(G);
										if (!(F || F === 0)) {
											var J = G.toString().match(
													/blur\(([0-9]+[A-z]+)\)/i);
											if (J) {
												F = J[1]
											} else {
							...
							prefixCheck : function(G) {
								...
								for (var F = 0, H = J.length; F < H; F++) {
									...
									if (F === 0) {
										...
										I = J[F]
												+ G.replace(/^\w/, function(K) {
													return K.toUpperCase()
												})
									}
									if (t
											.isString(E.State.prefixElement.style[I])) {
						...
						Values : {
							isCSSNullValue : function(F) {
								return (F == 0 || /^(none|auto|transparent|(rgba\(0, ?0, ?0, ?0\)))$/i
										.test(F))
							},
							getUnitType : function(F) {
								if (/(^(scale|scaleX|scaleY|scaleZ|alpha|flexGrow|flexHeight|zIndex|fontWeight)$)|((opacity|red|green|blue|alpha)$)/i
										.test(F)) {
									return ""
								} else {
									return "px"
							...
							getDisplayType : function(G) {
								...
								if (/^(b|big|i|small|tt|abbr|acronym|cite|code|dfn|em|kbd|strong|samp|var|a|bdo|br|img|map|object|q|script|span|sub|sup|button|input|label|select|textarea)$/i
										.test(F)) {
									return "inline"
								} else {
									if (/^(li)$/i.test(F)) {
						...
						getPropertyValue : function(I, K, H, J) {
							function G(P, U) {
								var Q = 0;
								var T = false;
								if (/^(width|height)$/.test(U)
										&& v.getPropertyValue(P, "display") === 0) {
									T = true;
									v.setPropertyValue(P, "display", v.Values
											.getDisplayType(P))
								}
								function O() {
									if (T) {
										v
												.setPropertyValue(P, "display",
														"none")
									}
								}
								if (!J) {
								...
								O();
								if (Q === "auto"
										&& /^(top|right|bottom|left)$/i.test(U)) {
									var R = G(P, "position");
									if (R === "fixed"
											|| (R === "absolute" && /top|left/i
													.test(U))) {
										Q = $(P).position()[U] + "px"
									}
								}
							...
							if (v.Normalizations.registered[K]) {
								...
								F = v.Normalizations.registered[K]("extract",
										I, M)
							}
							if (!/^[\d-]/.test(F)) {
						...
						setPropertyValue : function(I, J, F, H, K) {
							...
							if (v.Normalizations.registered[J]) {
								F = v.Normalizations.registered[J]("inject", I,
										F);
							...
							return typeof F === "object"
									&& /^\[object (HTMLCollection|NodeList|Object)\]$/
											.test(Object.prototype.toString
													.call(F))
									&& F.length !== undefined
									&& (F.length === 0 || (typeof F[0] === "object" && F[0].nodeType > 0))
						},
					...
					var n = function() {
						...
						var M = (arguments[0] && (arguments[0].p || ((q
								.isPlainObject(arguments[0].properties) && !arguments[0].properties.names) || t
								.isString(arguments[0].properties)))), P, F, G;
						var N, Q, J;
						P = true;
						G = 1;
						N = M ? (arguments[0].elements || arguments[0].e)
								: arguments[0];
						N = p(N);
						if (!N) {
							return
						...
						var Z = N.length, U = 0;
						if (!/^(stop|finish|finishAll)$/i.test(Q)
								&& !q.isPlainObject(J)) {
							var I = G + 1;
							J = {};
							for (var T = I; T < arguments.length; T++) {
								if (!t.isArray(arguments[T])
										&& (/^(fast|normal|slow)$/i
												.test(arguments[T]) || /^\d/
												.test(arguments[T]))) {
									J.duration = arguments[T]
								} else {
									if (t.isString(arguments[T])
											|| t.isArray(arguments[T])) {
											...
											.each(
													...
													function(ab, aa) {
														if (parseFloat(O.stagger)) {
															O.delay = Y
																	+ (parseFloat(O.stagger) * ab)
														} else {
															if (t
																	.isFunction(O.stagger)) {
																O.delay = Y
																		+ O.stagger
																				.call(
																						aa,
														...
														E.Redirects[Q]
																.call(
																		...
																		Z,
																		N,
																		promiseData.promise ? promiseData
																				: undefined)
													});
									return H()
								} else {
									var R = "jqxAnimations: First argument ("
											+ Q
						...
						function V() {
							...
							if (ad.display !== undefined && ad.display !== null) {
								ad.display = ad.display.toString()
										.toLowerCase();
								if (ad.display === "auto") {
									ad.display = E.CSS.Values
											.getDisplayType(aa)
								}
							}
							if (ad.visibility !== undefined
									&& ad.visibility !== null) {
								ad.visibility = ad.visibility.toString()
										.toLowerCase()
							}
							...
							function ae(ao) {
								...
								if (S === "start") {
									...
									function an(ay, ax) {
										...
										if (t.isArray(ay)) {
											...
											if ((!t.isArray(ay[1]) && /^[\d-]/
												...
												if ((t.isString(ay[1]) && !v.RegEx.isHex
														.test(ay[1]))
														|| t.isArray(ay[1])) {
													aA = ax ? ay[1] : w(ay[1],
															ad.duration);
									...
									for ( var av in Q) {
										...
										if (!m(aa).isSVG
												&& ak !== "tween"
												&& v.Names.prefixCheck(ak)[1] === false
												&& v.Normalizations.registered[ak] === undefined) {
											if (E.debug) {
												console
														.log("Skipping ["
																+ ak
										...
										if (((ad.display !== undefined
												&& ad.display !== null && ad.display !== "none") || (ad.visibility !== undefined && ad.visibility !== "hidden"))
												&& /opacity|filter/.test(av)
												&& !ag && at !== 0) {
											ag = 0
										}
							...
							if ((ad.queue === "" || ad.queue === "fx")
									&& q.queue(aa)[0] !== "inprogress") {
								q.dequeue(aa)
							}
						}
						q.each(N, function(ab, aa) {
					...
					function A(G) {
						if (G) {
							...
							for (var V = 0; V < Z; V++) {
									...
									if (O.display !== undefined
										...
										if (O.display === "flex") {
											...
											q.each(M, function(ac, ad) {
												v.setPropertyValue(F,
														"display", ad)
											})
										}
										v.setPropertyValue(F, "display",
												O.display)
									}
									if (O.visibility !== undefined
											&& O.visibility !== "hidden") {
										v.setPropertyValue(F, "visibility",
												O.visibility)
									}
									for ( var K in Q) {
										if (K !== "element") {
											var I = Q[K], N, R = t
													.isString(I.easing) ? E.Easings[I.easing]
													: I.easing;
											if (W === 1) {
												N = I.endValue
											} else {
								...
								if (O.progress) {
									O.progress.call(L[1], L[1], W, Math.max(0,
											(ab + O.duration) - U), ab, H)
								}
								if (W === 1) {
									B(V)
								}
						...
						for (var M = 0, H = R.length; M < H; M++) {
							...
							if (!J && !G.loop) {
								...
								if (G.visibility === "hidden") {
									v.setPropertyValue(L, "visibility",
											G.visibility)
								}
							}
							if (G.loop !== true
									&& (q.queue(L)[1] === undefined || !/\.jqxAnimationsQueueEntryFlag/i
											.test(q.queue(L)[1]))) {
								if (m(L)) {
					...
					for (var y = 0; y < 2; y++) {
						...
						var r = function(F) {
							q["slide" + F] = function(P, I, N, G, L) {
								...
								var H = q.extend({}, P), O = H.begin, M = H.complete, K = {
									height : "",
									marginTop : "",
									marginBottom : "",
									paddingTop : "",
								...
								if (H.display === undefined) {
									H.display = (F === "Down" ? (E.CSS.Values
											.getDisplayType(l) === "inline" ? "inline-block"
											: "block")
											: "none")
								}
								H.begin = function() {
			...
			.extend(
					c,
					{
						...
						add : function(n, r, z, o) {
							...
							if (!p) {
								s.handle = p = function(t) {
									return c.dispatch.apply(p.elem, new Array(
											t, q))
								};
								p.elem = n
							}
						...
						trigger : function(k, r, p, z) {
							...
							for (s = 0; s < q.length
									&& !k.isPropagationStopped(); s++) {
								x = q[s][0];
								k.type = q[s][1];
								t = (u.data(x, "events") || {})[k.type]
										&& u.data(x, "handle");
								if (t) {
									t.apply(x, r)
								}
								t = n && x[n];
								if (t && jQuery.acceptData(x) && t.apply
										&& t.apply(x, r) === false) {
									k.preventDefault()
								}
							}
						...
						dispatch : function(y, k) {
							...
							var o = this;
							y = c.createEvent(y);
							var A, x, q, B, z, s, n, l, w, C, u = ((r.data(o,
									"events") || {})[y.type] || []), t = u.delegateCount, p = Array.prototype.slice
									.call(arguments), v = !y.namespace, m = [];
							m.push({
								elem : this,
								matches : u.slice(t)
							});
							for (A = 0; A < m.length; A++) {
								...
								for (x = 0; x < s.matches.length; x++) {
									...
									if (v || (!y.namespace && !l.namespace)
											|| y.namespace_re
											&& y.namespace_re.test(l.namespace)) {
										y.data = l.data;
										y.handleObj = l;
...
(function(a) {
	...
	if (!Object.keys) {
		Object.keys = (function() {
			var d = Object.prototype.hasOwnProperty, e = !({
				toString : null
			}).propertyIsEnumerable("toString"), c = [ "toString",
					"toLocaleString", "valueOf", "hasOwnProperty",
					"isPrototypeOf", "propertyIsEnumerable", "constructor" ], b = c.length;
			return function(h) {
				if (typeof h !== "object"
	...
	a.jqx.jqxWidget = function(b, c, f) {
			...
			jqwidgets[g] = window[g] = function(e, r) {
				...
				if (window.jqxWidgets && window.jqxWidgets[n]) {
					...
					if (window.jqxWidgets[n].length == 1) {
						var p = a(
								window.jqxWidgets[n][0].widgetInstance.element)
								.data();
						if (p && p.jqxWidget) {
							return window.jqxWidgets[n][0]
						}
					}
					var p = a(window.jqxWidgets[n][0].widgetInstance.element)
							.data();
					if (p && p.jqxWidget) {
				...
				if (o.length === 0) {
					o = a("<div></div>");
					if (g === "jqxInput" || g === "jqxPasswordInput"
							|| g === "jqxMaskedInput") {
						o = a("<input/>")
					}
					if (g === "jqxTextArea") {
					...
					if (g === "jqxButton" || g === "jqxRepeatButton"
							|| g === "jqxToggleButton") {
						o = a("<button/>")
					}
					if (g === "jqxSplitter") {
						...
						.each(
								...
								function(v) {
									...
									if (!k[g]) {
										...
										var w = a.jqx["_" + g].prototype
												.defineInstance();
										var u = {};
										if (a.jqx["_" + g].prototype.metaInfo) {
											u = a.jqx["_" + g].prototype
													.metaInfo()
										}
										if (g == "jqxDockingLayout") {
											w = a.extend(w,
													a.jqx._jqxLayout.prototype
															.defineInstance())
										}
										if (g == "jqxToggleButton"
												|| g == "jqxRepeatButton") {
											w = a.extend(w,
		...
		try {
			a.extend(a.jqx["_" + g].prototype,
					Array.prototype.slice.call(f, 0)[0])
		} catch (h) {
		}
		a.extend(a.jqx["_" + g].prototype, {
			...
			.extend(
					a.jqx.utilities,
					{
							...
							var m = function(v, E) {
								...
								var w = function(J) {
									if (k.hiddenWidgets.length > 0) {
										...
										k.__resizeInterval = setInterval(
												function() {
													I()
												}, 100)
									}
									...
									if (a.jqx.isHidden(D)) {
										...
										if (v === undefined || v !== true) {
											...
											if (A.jqxWidget.element) {
												...
												if (x.indexOf("dropdownlist") >= 0
														|| x
																.indexOf("datetimeinput") >= 0
														|| x
																.indexOf("combobox") >= 0
														|| x.indexOf("menu") >= 0) {
								...
								if (j) {
									...
									var f = function(t) {
										if (k.watchedElementData.forEach) {
											k.watchedElementData
													.forEach(function(u) {
														if (u.element.offsetWidth !== u.offsetWidth
						...
						parseJSON : function(d) {
							...
							if (b.test(d.replace(c, "@").replace(e, "]")
									.replace(f, ""))) {
								...
								return a
										.access(
												...
												function(s) {
													...
													if (typeof s === "string"
															&& !r.test(s)
															&& (a.support.htmlSerialize || !k
																	.test(s))
															&& (a.support.leadingWhitespace || !q
																	.test(s))
															&& !t[(p.exec(s) || [
																	"", "" ])[1]
																	.toLowerCase()]) {
					...
					a.jqx.mobile,
					{
						...
						isTouchDevice : function() {
							...
							try {
								...
								if (navigator.platform.toLowerCase().indexOf(
										"win") != -1) {
									if (f.indexOf("Windows Phone") >= 0
											|| f.indexOf("WPDesktop") >= 0
						...
						getTopPos : function(c) {
							...
							if (this.isSafariMobileBrowser()) {
								...
								if (d.indexOf("version/6") != -1
										|| d.indexOf("version/5") != -1) {
									e = e + a(window).scrollTop()
								}
								if (/(Android.*Chrome\/[.0-9]* (!?Mobile))/
						...
						touchScroll : function(A, L, V, G, w, m) {
							...
							var G = G;
							var t = this.getTouchEventName("touchstart")
									+ ".touchScroll";
							var C = this.getTouchEventName("touchend")
									+ ".touchScroll";
							var X = this.getTouchEventName("touchmove")
									+ ".touchScroll";
							...
							function z(ag) {
								if (ag.targetTouches
										&& (ag.targetTouches.length >= 1)) {
									return ag.targetTouches[0].clientY
								} else {
									if (ag.originalEvent
											&& ag.originalEvent.clientY !== undefined) {
							...
							function aa(ag) {
								if (ag.targetTouches
									...
									if (ag.originalEvent
											&& ag.originalEvent.clientX !== undefined) {
							...
							var T = function(ag) {
								...
								if (a.inArray(ag.target.tagName.toLowerCase(),
										P) !== -1) {
									return
								}
								...
								var ai = ah[0];
								if (ah.length == 1) {
									F.dispatchMouseEvent("mousedown", ai, F
											.getRootNode(ai.target))
								}
								U = w.jqxScrollBar("max");
							...
							var Y = function(ag, af) {
								...
								V(null, ag, 0, 0, af);
								return (ag > O) ? "max" : (ag < ad) ? "min"
										: "value"
							};
							var l = function(ag, af) {
								xoffset = (ag > U) ? U : (ag < ad) ? ad : ag;
								V(ag, null, 0, 0, af);
								return (ag > U) ? "max" : (ag < ad) ? "min"
										: "value"
							};
							function R() {
								var af, ag;
							...
							var x = function(af) {
								...
								e = am;
								g = an;
								var ah = w != null ? w[0].style.visibility != "hidden"
										: true;
								var al = m != null ? m[0].style.visibility != "hidden"
										: true;
								function aj(aq) {
									var at, ar, ap;
									if (E) {
							...
							var r = function(ag) {
								...
								F.scrolling[G] = false;
								if (j) {
									F.dispatchMouseEvent("mouseup", ah,
											ag.target)
								} else {
									var ah = F.getTouches(ag)[0], af = F
											.getRootNode(ah.target);
							...
							if (this.simulatetouches) {
								...
								a(window).on != undefined ? a(document).on(
										"mouseup.touchScroll", N) : a(document)
										.bind("mouseup.touchScroll", N);
								...
								var ae = a(document).on != undefined
										|| a(document).bind;
								var J = function(af) {
									if (!F.scrolling[G]) {
										return true
							...
							o.on ? o.on(C + " touchcancel.touchScroll", r) : o
									.bind(C + " touchcancel.touchScroll", r)
						}
					});
	...
	a.extend(a.jqx.string, {
		...
		containsIgnoreCase : function(b, c) {
			...
			return b.toString().toUpperCase().indexOf(
					c.toString().toUpperCase()) != -1
		},
		equals : function(b, c) {
			if (b == null || c == null) {
				return false
...
(function(b) {
	...
	b.fn.coord = function(o) {
		...
		var d = function(p) {
			return b.isWindow(p) ? p : p.nodeType === 9 ? p.defaultView
